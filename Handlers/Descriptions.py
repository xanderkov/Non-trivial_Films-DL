...
class Model(object):
    ...
    def getDistance(self, first, second):
        return self.model.wmdistance(first, second)


def moreSimilar(films, id, prepDisc, model, done_ids):
    lst = []
    for i in films:
        if (i[0] != id) and (not(id in done_ids)):
...
def prep_dist(films, model, db):
    ...
    for i in tqdm(films):
        if not (i[0] in done_ids):
            dist = moreSimilar(films, i[0], i[1], model, done_ids)
